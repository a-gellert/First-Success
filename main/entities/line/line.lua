...
function Line.new(game_object_id, origin_pos, line_width, scale_collider, resolution)
	...
	instance.line_width = line_width or 6
...
function Line:set_position(pos)
	...
	-- Обновление отображения (_renderer: LineRenderer) в реальном времени
	if #self.points >= 2 then
		local buf = mesh_builder.create_line_buffer(self.points, self.line_width, self.origin_pos)
		if buf then
	...
	-- Обновление коллайдера (_collider.points = _points.ToArray(); _collider.edgeRadius = scaleCollider;)
